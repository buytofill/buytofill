<?php
    ...
    if(isset($_POST['email'])){
        ...
        list($email, $phone, $pwd, $rpwd, $fn, $ln, $company, $site) = array($_POST['email'], $_POST['phone'], $_POST['pwd'], $_POST['rpwd'], $_POST['fn'], $_POST['ln'], $_POST['company'], $_POST['site']);
        ...
        $client->sendEmail([
            ...
            'Source' =>'noreply@buytofill.com'
        ]);
        $_SESSION['st'] = time(); 
        
        list($_SESSION['uEmail'], $_SESSION['pwd'], $_SESSION['fn'], $_SESSION['ln'], $_SESSION['phone'], $_SESSION['company'], $_SESSION['site']) = array($email, $pwd, $fn, $ln, $phone, $company, $site);
    ...
    if(isset($_POST['emailedCode'])){
        ...
        if($emailedCode == $_SESSION['emailedCode'] && $_SESSION['emailedCode'] != "" && time() - $_SESSION['st'] < 300){
            ...
            $stmt = $conn->prepare("INSERT INTO `buyer` (fn, ln, email, pass, company, site, phone) SELECT ?, ?, ?, ?, ?, ?, ? FROM dual WHERE NOT EXISTS (SELECT 1 FROM `buyer` WHERE (email = ? OR phone = ?) AND level IS NOT NULL)");
            $stmt->bind_param("sssssssss", $_SESSION['fn'], $_SESSION['ln'], $_SESSION['uEmail'], $pass, $_SESSION['company'], $_SESSION['site'], $_SESSION['phone'], $_SESSION['uEmail'], $_SESSION['phone']);
            if($stmt->execute()){
                if($conn->affected_rows > 0){
                    $conn->close();
                    say("We've recieved your request, we'll be in contact within 24 business hours.",200);
                } else {
                    $conn->close();
                    say("Email or Phone is already registered.",400);
                }
            }
            $conn->close();
        }else{
...
<html lang="en">
    ...
    <body>
        ...
        <div style="display:flex;flex-direction:column;justify-content:center;align-items:center;gap:1rem;position:absolute;">
            <div class="suh">
                ...
                <h4 style="color: var(--purple);font-size:1.5rem; width:100%;margin-bottom:-.5rem;">Buyer Sign Up</h4>
            ...
            <div class="duh">
                <form onsubmit="proceed(event,this)">
                    <div class="cutenput">
                        <div class="enput" style="width:90%">
                    ...
                    <div class="cutenput">
                        <div class="enput" style="width:150%">
                    ...
                    <div class="cutenput">
                        <div class="enput"style="width:80%">
                            <input name="company" type="text" placeholder="" autocomplete="site">
                            <span>Company (Optional)</span>
                        </div>
                        <div class="enput">
                            <input name="site" type="text" placeholder="" autocomplete="site">
                            <span>Website (Optional)</span>
                        </div>
                    </div>
                    <div>
                        <div style="display:flex; flex-direction:column;gap:0;align-items:revert;">
                            <a href="signup">Not a buyer? <span>Register as Filler</span></a>
                            <a href="login">Already have an account? <span>Login</span></a>
                        </div>
                        <div><p>1/2</p><input type="submit" value="Proceed"></div>
                    </div>
                </form>
    ...
    <script>
        ...
        function vEmail(e,a){
            if(!a){
            ...
            }else if(e.submitter.value == "Request Account"){
                e.preventDefault();
                const formData = new FormData(a);
                fetch('#',{method:'POST',body:formData})
                ...
                .then(data => {
                    sendNotif(data[0],data[1]);
                    document.querySelector('body').classList = "done";
                });
            }
        }
        /*function vPhone(e,a){
            if(!a){
                ...
                .then(data => {
                    sendNotif(data[0],data[1]);
                    if(data['1'] == 200){
                        window.location.href = "https://buytofill.com/dashboard?tutorial=true"
                    }
                });
            }
        }*/
